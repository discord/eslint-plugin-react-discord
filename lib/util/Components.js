...
function componentRule(rule, context) {
  ...
  const utils = {
    ...
    getStatelessComponent(node) {
      ...
      // Discord patch:
      // If a component is just a single expression, it's not a block, then lets not consider it a react component,
      // it is probably more likely a render function. This is a component that also can't be using any hooks either
      // otherwise it'd have a block body, so it can't be anything tooo interesting.
      if (node.type === 'ArrowFunctionExpression' && (node.body == null || node.body.type !== 'BlockStatement')) {
        return;
      }
